...
def plot_s_parameters(freq, s_params, title=None):
    """
    Create plots for S-parameters with markers at 433MHz and 868MHz
    """
    ...
    marker_colors = ['g', 'm']
    marker_names = ['433 MHz', '868 MHz']
    marker_styles = ['o', 's']  # circle and square
    
    # Helper function to add markers to a plot
    def add_markers(ax, freq_data, mag_data, phase_data):
    ...
    # Add markers to S11
    add_markers(axs[0, 0], freq, s_params['S11']['mag'], s_params['S11']['phase'])
    ...
    # Add markers to S21
    add_markers(axs[0, 1], freq, s_params['S21']['mag'], s_params['S21']['phase'])
    ...
    # Add markers to S12
    add_markers(axs[1, 0], freq, s_params['S12']['mag'], s_params['S12']['phase'])
    ...
    # Add markers to S22
    add_markers(axs[1, 1], freq, s_params['S22']['mag'], s_params['S22']['phase'])
    
    # Add a common legend for all subplots
    handles = []
    labels = []
    for i, marker_freq in enumerate(marker_freqs):
        handle = plt.Line2D([0], [0], marker=marker_styles[i], color=marker_colors[i],
                           markersize=8, linestyle='none')
        handles.append(handle)
        labels.append(marker_names[i])
    
    fig.legend(handles, labels, loc='upper right', bbox_to_anchor=(0.99, 0.99), fontsize=10)
    
    plt.tight_layout()
    fig.subplots_adjust(top=0.9, right=0.95)  # Make room for the legend
    plt.show()
...
def main():
    ...
    try:
        ...
        # Check if marker frequencies are within range
        freq_min, freq_max = min(freq), max(freq)
        marker_freqs = [433, 868]  # MHz
        valid_markers = []
        
        print(f"Frequency range in file: {freq_min:.2f} MHz to {freq_max:.2f} MHz")
        
        for marker in marker_freqs:
            if freq_min <= marker <= freq_max:
                valid_markers.append(f"{marker} MHz (in range)")
            else:
                closest_freq = freq[np.abs(freq - marker).argmin()]
                valid_markers.append(f"{marker} MHz (closest point: {closest_freq:.2f} MHz)")
        
        print("Marker information:")
        for marker_info in valid_markers:
            print(f"- {marker_info}")
